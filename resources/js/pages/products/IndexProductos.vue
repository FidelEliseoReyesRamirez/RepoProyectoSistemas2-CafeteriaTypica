...
const productosFiltrados = computed(() => {
    return props.productos.filter(p => {
        ...
        return coincideNombre && coincideCategoria && coincidePrecio && coincideDisponible && coincideAgotado;
    });
});
...
<template>
    ...
    <div class="p-3 sm:p-6 text-[#4b3621] dark:text-white w-full max-w-full overflow-x-hidden">
        ...
        <div class="w-full flex flex-wrap gap-2 sm:gap-4 mb-4">
            ...
            <button
                @click="() => { filtroNombre = ''; filtroCategoria = ''; filtroPrecioMax = ''; filtroSoloDisponibles = false; filtroSoloAgotados = false; }"
                ...
                <div v-for="producto in productos" :key="producto.id_producto" class="relative perspective">
                    <div :class="['transition-transform duration-500 transform-style-preserve-3d', 'w-full h-auto min-h-[270px] sm:min-h-[280px]',
                        ...
                        <div
                            ...
                            <div>
                                <p class="text-xs font-semibold text-center mb-1" :class="[producto.cantidad_disponible === 0 || !producto.disponibilidad ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400']">
                                    Estado: {{
                                ...
                                <div class="flex justify-between gap-2">
                                    <button
                                        ...
                                        :class="['text-white text-[11px] font-medium py-1 px-2 rounded shadow w-1/2', producto.cantidad_disponible === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-yellow-500 hover:bg-yellow-600']">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-2 sm:p-4">
        <div class="bg-white dark:bg-[#2c211b] p-4 sm:p-6 rounded-lg w-full max-w-sm shadow-xl text-sm">
            ...
            <p class="mb-4">¿Estás seguro que deseas eliminar el producto <strong>{{ selectedProductName }}</strong>?</p>
