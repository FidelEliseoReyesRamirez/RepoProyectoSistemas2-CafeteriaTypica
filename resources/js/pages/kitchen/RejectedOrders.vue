...
const filteredOrders = computed(() =>
    orders.value.filter(p => p.estadopedido.nombre_estado === 'Rechazado')
)

const restaurarPedido = async (id: number) => {
    try {
        await axios.post(`/pedidos/${id}/restaurar`)
        loadOrders()
    } catch (error) {
...
<template>
    <AppLayout>
        ...
        <Head title="Pedidos Rechazados" />
        <div class="p-4 sm:p-6 w-full max-w-screen-xl mx-auto text-[#4b3621] dark:text-white">
            <h1 class="text-2xl sm:text-3xl font-bold mb-6">Pedidos Rechazados</h1>

            <button @click="goBack" class="mb-6 px-4 py-2 text-sm rounded shadow text-white bg-gray-600">
                Volver
            </button>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="pedido in filteredOrders" :key="pedido.id_pedido"
                    class="rounded-lg shadow border relative overflow-hidden p-4" :style="{
                        backgroundColor: `${pedido.estadopedido.color_codigo}22`,
                        borderColor: pedido.estadopedido.color_codigo
                    }">
                    ...
                    <div class="mt-4 text-center">
                        <button @click="restaurarPedido(pedido.id_pedido)"
