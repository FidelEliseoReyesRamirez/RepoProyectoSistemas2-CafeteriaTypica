...
const filteredOrders = computed(() =>
  orders.value.filter(p =>
    ['Pagado'].includes(p.estadopedido.nombre_estado) &&
...
<template>
  <AppLayout>
    <Head title="Pedidos Finalizados" />
    <div class="p-4 sm:p-6 w-full max-w-screen-xl mx-auto text-[#4b3621] dark:text-white">
      <h1 class="text-2xl sm:text-3xl font-bold mb-6">Pedidos Finalizados</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="pedido in filteredOrders" :key="pedido.id_pedido"
          ...
          <p class="text-xs font-semibold mb-2">
            Estado:
            <span class="rounded px-2 py-0.5 text-white" :style="{ backgroundColor: pedido.estadopedido.color_codigo }">
